...
function FollowUpRequests() {
  ...
  const { initiateCall, answerCall, activeCall } = useCall();

  useEffect(() => {
    fetchFollowUpRequests();
  }, []);

  const handleJoinCall = async (request) => {
    try {
      const user = JSON.parse(localStorage.getItem('user'));
      const doctorId = request.doctor?._id || request.doctorId;
      const doctorName = request.doctor?.name || 'Doctor';
      
      console.log('🔗 Joining call with:', doctorName, doctorId);
      
      // Answer the call if there's an incoming call for this doctor
      // Otherwise, initiate a call to the doctor
      await initiateCall(doctorId, doctorName, 'video', 'en');
      toast.success(`Connecting to Dr. ${doctorName}...`);
    } catch (error) {
          ...
          {requests.map((request) => (
            <div key={request._id} className={`request-card ${request.status}`}>
              ...
              {request.status === 'accepted' && 
               (request.requestType === 'audio_call' || request.requestType === 'video_call') && 
               !activeCall && (
                <div className="request-actions">
                  <button 
                  ...
                  >
                    {request.requestType === 'audio_call' ? <FiPhone /> : <FiVideo />}
                    Join Call
                  </button>
                </div>
              )}
